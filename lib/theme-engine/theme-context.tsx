...
import type { ThemeConfig, DeviceType, DeviceCapabilities } from '@/types/theme'
import { defaultColors, defaultSpacing, defaultTypography, defaultGradients, defaultShadows, defaultBorder, defaultEffects, defaultAnimations, defaultBackgrounds, generateCSSVariables } from './tokens'
    ...
    const detectDevice = (): DeviceCapabilities => {
      ...
      const capabilities = {
        ...
        prefersColorScheme: (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' :
                           window.matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'no-preference') as any
      }
